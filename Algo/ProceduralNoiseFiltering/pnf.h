...
inline IMG computeIMG(const Vec2 & w_size, const Vec2 &im_size, const func &f){
    Mat22 borns;
...
inline ImageGray<T> compute_noise_unfiltered(const Vec2 & w_size,
                                  const Vec2 &im_size,
                                  const Noise<T> &n)
{
    return computeIMG<ImageGray<T>>(w_size, im_size, [&](const Vec2 &pos) {
        return n.basic2D(pos);
    });
}

inline ImageGray<T> compute_noise_filtered(const Vec2 & w_size,
                                            const Vec2 &im_size,
                                            const Noise<T> &n,
                                            const int &nb_sample)
{
...
//texture noise
template<typename IMG, typename func>
inline IMG computeIMG(const Vec2 &offset, const Vec2 & w_size, const Vec2 &im_size, const func &f){
    Mat22 borns;
    borns << w_size(0), 0, 0, w_size(1);
