...
void PatchProcessor<I>::ChooserContents_PCTSEnhanced<CC_Base>::generate()
{
    ...
    for(int i=0; i<this->m_patches.size(); ++i)
    {
        ...
        // mask.save(std::string(TEMPO_PATH+"ieee2019/_debug/mask_p") + std::to_string(i) + ".png");
...
//           contentTexture.save(std::string(TEMPO_PATH+"ieee2019/_debug/inputWithContent_p") + std::to_string(i) + "_c" + std::to_string(j) + ".png");
            pcts.setMask(contentTexture, mask);
            contentTexture = pcts.generate();
//          contentTexture.save(std::string(TEMPO_PATH+"ieee2019/_debug/pctsd_p") + std::to_string(i) + "_c" + std::to_string(j) + ".png");
