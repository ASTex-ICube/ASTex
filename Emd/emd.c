	  ...
	  flow_t *Flow, int *FlowSize)
{
  ...
  node2_t *XP;
  flow_t *FlowP;
  node1_t *U = (node1_t *)malloc(MAX_SIG_SIZE1*sizeof(node1_t));
  node1_t *V = (node1_t *)malloc(MAX_SIG_SIZE1*sizeof(node1_t));
		  ...
		  float (*Dist)(feature_t *, feature_t *))
{
  ...
  double *S = (double*)malloc(_n1*sizeof(double));
  double *D = (double*)malloc(_n2*sizeof(double));
  _X=(node2_t*)malloc((_n1+_n2)*sizeof(node2_t));

  _C = (float**)malloc(_n1*sizeof(float*));
  int __i;
  for(__i=0; __i<_n1; ++__i)
      _C[__i] = (float*)malloc(_n2*sizeof(float));

  _IsX = (char **)malloc(_n1*sizeof(char *));
  for(__i=0; __i<_n1; ++__i)
      _IsX[__i] = (char*)malloc(_n2*sizeof(char));

  _RowsX=(node2_t**) malloc(_n1 * sizeof(node2_t*));
  _ColsX=(node2_t**) malloc(_n2 * sizeof(node2_t*));
...
static void russel(double *S, double *D)
{
  ...
  Delta = (double**)malloc(MAX_SIG_SIZE1*sizeof(double*));
  for(i=0; i<MAX_SIG_SIZE1; ++i)
  {
    Delta[i]=(double*)malloc(MAX_SIG_SIZE1*sizeof(double));
  }
...
static void freeGlobal(signature_t *Signature1)
{
    free(_X);

    int __i;
    for(__i=0; __i<Signature1->n; ++__i)
        free(_C[__i]);
    free(_C);
